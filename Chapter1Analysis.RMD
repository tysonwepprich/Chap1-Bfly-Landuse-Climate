...
rsa <- c(0, 10, 20)
models <- c("dens", "clim", "land", "d.c", "d.l", "d.c.l.add", "d.c.l.int")
...
modlist[["clim"]] <- as.formula(growth_ukbms_add1 ~ zmean * (siteanom_currsum_meanTemp + 
                                                               siteanom_spring_meanTemp + 
                                                               siteanom_winter_meanTemp + 
                                                               siteanom_fall_meanTemp +  
                                                               siteanom_prevsum_meanTemp +
...
modlist[["land"]] <- as.formula(growth_ukbms_add1 ~ PC1*PC2)

modlist[["d.c"]] <- as.formula(growth_ukbms_add1 ~ ddsite_ukbms +
                                 ddsite_ukbms_square + 
                                 zmean * (siteanom_currsum_meanTemp + 
                                            siteanom_spring_meanTemp + 
                                            siteanom_winter_meanTemp + 
...
modlist[["d.l"]] <- as.formula(growth_ukbms_add1 ~ ddsite_ukbms +
                                 ddsite_ukbms_square + PC1*PC2)
...
argsdf <- expand.grid(spec2model, rsa, models)
argslist <- as.list(1:nrow(argsdf))
# make stloo fitting a parallel function
argstrain <- expand.grid(spec2model, rsa)
argstrainlist <- as.list(1:nrow(argstrain))


getTraining <- function(rownum){
  temp <- argstrain[rownum, ]
  spec <- temp[1, 1]
  rsa <- temp[1, 2]
  datsp <- datmodcut %>% filter(sp == spec) 
  datsp <- as.data.frame(datsp)
  training.sets <- splittingST(data = datsp, 
                               lon.lab="lon",
                               lat.lab="lat",
                               yr.lab = "Year",
                               dist.split=rsa,
...
# # Initiate cluster
# cl <- makeCluster(no_cores)
# clusterEvalQ(cl, {source("spatiotempLOO.R"); library(dplyr)})
# clusterExport(cl, c("datmodcut", "argstrain"))
# trainList <- parLapply(cl, argstrainlist, getTraining)
# stopCluster(cl)

# saveRDS(trainList, "LOOtrainingsets.rds")
trainList <- readRDS("LOOtrainingsets.rds")

library(MASS)
library(nlme)

fitstloo <- function(rownum){
  
  temp <- argsdf[rownum, ]
  spec <- temp[1, 1]
  RSA <- temp[1, 2]
  index <- as.character(temp[1,3])
  modcall <<- modlist[[index]]
  
  training.set <- trainList[[which(argstrain[,1] == spec & argstrain[,2] == RSA)]]
  
  datsp <<- datmodcut %>% filter(sp == spec) 
  datsp <- as.data.frame(datsp)
  
  mfull <- gls(modcall, 
               data = datsp, correlation = corExp(form = ~ lat + lon | YearFact), method = "ML")

  test <- modSelect(full.mod = mfull, data = datsp, training = training.set, steps = 100)
  
  test$stepmod <- stepAIC(mfull, direction = "backward", test = "Chisq", trace = 0)
  return(test)
}
...
clusterExport(cl, c("datmodcut", "argsdf", "argstrain", "trainList", "modlist"), envir = .GlobalEnv)
datList <- parLapply(cl, argslist[c(1, 100, 500, 800)], fitstloo)
stopCluster(cl)
...
results <- data.frame()
test <- datList
  for (m in 1:length(argslist)){
    species <- argsdf[m, 1]
    rsa <- argsdf[m, 2]
    temp <- data.frame(model = m, species = species, rsa = rsa, loglik = test[[m]]$loglik, 
                       npar = length(coef(test[[m]]$fit)), aic = test[[m]]$AIC, 
                       npar_step = length(coef(test[[m]]$stepmod)), 
                       aic_step = AIC(test[[m]]$stepmod))
    results <- rbind(results, temp)
    # names(results) <- c("model", "rsa", "loglik", "npar", "AIC")
  }

results %>% arrange(species, rsa)
```
